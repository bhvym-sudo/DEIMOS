...
func TweetDetailToExcel(record model.TweetDetailRecord, output string) error {
	if record.Tweet.ID == "" {
		return fmt.Errorf("tweet detail data is empty")
	}
	if strings.ToLower(filepath.Ext(output)) != ".xlsx" {
		output += ".xlsx"
	}
	file := excelize.NewFile()
	file.SetSheetName("Sheet1", "Tweet Detail")
	writeHeader(file, "Tweet Detail", []string{"Section", "Field", "Value"})
	row := 2
	summary := []model.Field{
		{Label: "Tweet ID", Value: record.Tweet.ID},
		{Label: "Created At", Value: record.Tweet.CreatedAt},
		{Label: "Tweet URL", Value: record.Tweet.URL},
		{Label: "Author", Value: record.Tweet.Author["screen_name"]},
		{Label: "Author Name", Value: record.Tweet.Author["name"]},
		{Label: "Text", Value: record.Tweet.Text},
	}
	for _, field := range summary {
		_ = file.SetCellValue("Tweet Detail", fmt.Sprintf("A%d", row), "Summary")
		_ = file.SetCellValue("Tweet Detail", fmt.Sprintf("B%d", row), field.Label)
		_ = file.SetCellValue("Tweet Detail", fmt.Sprintf("C%d", row), valueText(field.Value))
		row++
	}
	for _, section := range record.Sections {
		for _, field := range section.Rows {
			_ = file.SetCellValue("Tweet Detail", fmt.Sprintf("A%d", row), section.Title)
			_ = file.SetCellValue("Tweet Detail", fmt.Sprintf("B%d", row), field.Label)
			_ = file.SetCellValue("Tweet Detail", fmt.Sprintf("C%d", row), valueText(field.Value))
			row++
		}
	}
	addRawSheet(file, "Raw Tweet", record.Tweet.Raw)
	styleWorkbook(file, "Tweet Detail", 3, row-1)
	return file.SaveAs(output)
}
