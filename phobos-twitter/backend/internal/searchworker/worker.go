...
// Account captures profile data inside the authenticated Edge session when
// X rejects direct replay of the saved cookies.
func (c *Collector) Account(ctx context.Context, screenName string) (map[string]any, error) {
	c.browserMu.Lock()
	defer c.browserMu.Unlock()
	scriptPath := strings.TrimSpace(os.Getenv("TREX_ACCOUNT_WORKER"))
	if scriptPath == "" {
		scriptPath = filepath.Join(c.root, "python_worker", "account_lookup.py")
	}
	if !fileExists(scriptPath) {
		return nil, fmt.Errorf("account browser worker is missing at %s", scriptPath)
	}
	python := strings.TrimSpace(os.Getenv("TREX_PYTHON"))
	arguments := []string{"-u", scriptPath, "--profile", c.profileDir, "--username", screenName}
	if runtimePath := strings.TrimSpace(os.Getenv("TREX_SESSION_RUNTIME")); runtimePath != "" {
		arguments = []string{"-u", scriptPath, "--runtime", runtimePath, "--username", screenName}
	}
	if python == "" {
		if value, err := exec.LookPath("python"); err == nil {
			python = value
		} else if value, err := exec.LookPath("py"); err == nil {
			python = value
			arguments = append([]string{"-3"}, arguments...)
		} else {
			return nil, fmt.Errorf("Python was not found")
		}
	}
	...
	if err := cmd.Run(); err != nil {
		...
		return nil, fmt.Errorf("browser account lookup failed: %s", detail)
	}
	result := map[string]any{}
	if err := json.Unmarshal(stdout.Bytes(), &result); err != nil {
		return nil, fmt.Errorf("decode browser account response: %w", err)
	}
	profile, _ := result["profile"].(map[string]any)
	if len(profile) == 0 {
		return nil, fmt.Errorf("browser did not capture the X profile response")
	}
	return profile, nil
}
...
func (c *Collector) command() (string, []string, error) {
	sessionArguments := []string{"--profile", c.profileDir}
	if runtimePath := strings.TrimSpace(os.Getenv("TREX_SESSION_RUNTIME")); runtimePath != "" {
		sessionArguments = []string{"--runtime", runtimePath}
	}
	if executable := strings.TrimSpace(os.Getenv("TREX_SEARCH_WORKER_EXE")); executable != "" {
		...
		return executable, sessionArguments, nil
	}
	if fileExists(c.scriptPath) {
		if configured := strings.TrimSpace(os.Getenv("TREX_PYTHON")); configured != "" {
			return configured, append([]string{"-u", c.scriptPath}, sessionArguments...), nil
		}
		if python, err := exec.LookPath("python"); err == nil {
			return python, append([]string{"-u", c.scriptPath}, sessionArguments...), nil
		}
		if launcher, err := exec.LookPath("py"); err == nil {
			return launcher, append([]string{"-3", "-u", c.scriptPath}, sessionArguments...), nil
		}
