...
func New(runtimePath, profileDir string, logger *logging.Logger) *Manager {
	manager := &Manager{
		...
		sourcePath:  strings.TrimSpace(os.Getenv("TREX_SESSION_SOURCE")),
...
func (m *Manager) Status() model.SessionStatus {
	sourceErr := m.refreshSource()
	profileReady := m.profileHasSessionData()
	m.mu.RLock()
	defer m.mu.RUnlock()
	hasAuth := m.cookieLocked("auth_token") != ""
	hasCSRF := m.cookieLocked("ct0") != ""
	hasBearer := m.runtime.Bearer != ""
	ready := hasAuth && hasCSRF && hasBearer && sourceErr == nil
	message := "PHOBOS-Tweeter session is ready."
	if sourceErr != nil {
		message = "The copied X session is missing or unreadable. Copy runtime.json into phobos-twitter/sessions/x_session.json."
	} else if !ready {
		if !profileReady && len(m.runtime.Cookies) == 0 {
			message = "No local X browser session was found. Opening Edge for login."
		} else {
			message = "X session metadata is incomplete. Refresh it using the PHOBOS-Tweeter session utility."
		}
	}
...
func (m *Manager) Runtime() (Runtime, error) {
	if err := m.refreshSource(); err != nil {
		return Runtime{}, fmt.Errorf("load copied X session: %w", err)
	}
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *Manager) load() error {
	...
	} else if err := json.Unmarshal(data, &m.runtime); err != nil {
		return err
	}
	return m.refreshSource()
}

// refreshSource overlays credentials captured by the installed Electron app
// while retaining GraphQL operation metadata from the project runtime file.
// It runs for every request, so newly captured credentials are picked up.
func (m *Manager) refreshSource() error {
	if m.sourcePath == "" || filepath.Clean(m.sourcePath) == filepath.Clean(m.runtimePath) {
		return nil
	}
	...
	defer m.mu.Unlock()
	if source.Bearer != "" {
		m.runtime.Bearer = source.Bearer
	}
	if len(source.Cookies) > 0 {
		m.runtime.Cookies = append([]Cookie(nil), source.Cookies...)
	}
	if source.UserAgent != "" {
		m.runtime.UserAgent = source.UserAgent
	}
	if !source.CapturedAt.IsZero() {
		m.runtime.CapturedAt = source.CapturedAt
	}
	if source.ScreenName != "" {
		m.runtime.ScreenName = source.ScreenName
	}
	if len(source.Operations) > 0 {
		m.runtime.Operations = source.Operations
	}
	return nil
}
