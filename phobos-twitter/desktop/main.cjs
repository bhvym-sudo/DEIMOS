...
async function downloadUpdate() {
  ...
  downloadedUpdatePath = installerPath;
  sendUpdateEvent({ type: "ready-to-install", newVersion: updateState.version });
  return { downloaded: true, version: updateState.version };
}

async function installDownloadedUpdate() {
  if (!downloadedUpdatePath || !existsSync(downloadedUpdatePath)) {
    throw new Error("Downloaded installer is missing. Download the update again.");
  }
  const updatesDir = join(app.getPath("userData"), "updates");
  const scriptPath = join(updatesDir, "install-update.cmd");
  const script = [
    "@echo off",
    "timeout /t 2 /nobreak >nul",
    `start /wait "" "${downloadedUpdatePath}" /S`,
    "exit /b %errorlevel%"
  ].join("\r\n");
  require("node:fs").writeFileSync(scriptPath, script, "utf8");
  sendUpdateEvent({ type: "installing-after-close", newVersion: updateState?.version || "" });
  await requestBackendShutdown();
  const installer = spawn("cmd.exe", ["/c", scriptPath], {
    detached: true,
    windowsHide: true,
    stdio: "ignore"
  });
...
ipcMain.handle("updater:install-downloaded", async () => {
  try {
