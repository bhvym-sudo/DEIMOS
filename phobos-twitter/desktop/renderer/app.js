...
function setAppVersion(version) {
  ...
  $("#startup-version").textContent = label;
  $("#header-version").textContent = label;
}
...
function bindUI() {
  ...
  $("#tweet-detail-excel").addEventListener("click", exportTweetDetailExcel);
  $("#tweet-detail-pdf").addEventListener("click", exportTweetDetailPDF);
...
function renderTrackerPosts() {
  ...
  $("#tracker-count").textContent = state.trackerPosts.length;
  $("#tracker-posts").innerHTML = state.trackerPosts.slice(0, 20).map(post => postCardHTML(post, "tracker")).join("") || '<div class="empty-state">New tracked posts will stream here.</div>';
  $$("#tracker-posts .post-card").forEach(card => card.addEventListener("click", event => handlePostAction(card.dataset.post, event, "tracker")));
}
...
function renderPosts(posts, total) {
  ...
  $("#posts-list").innerHTML = posts.map(post => postCardHTML(post, "scan")).join("");
  $$("#posts-list .post-card").forEach(card => card.addEventListener("click", event => handlePostAction(card.dataset.post, event, "scan")));
}

function postCardHTML(post, source) {
  const username = post.author?.screen_name || "unknown";
  const sourceLabel = source === "tracker" ? "TRACKED POST" : "SEARCH RESULT";
  return `<article class="post-card" data-post="${escapeHTML(post.id)}" data-source="${escapeHTML(source)}">
    <div class="post-header">
      <span class="post-author">@${escapeHTML(username)}</span>
      <span class="post-time">${escapeHTML(displayPostTime(post.createdAt))}</span>
    </div>
    <div class="post-text">${escapeHTML(post.text || "")}</div>
    <div class="post-actions">
      <button data-action="comments">Tweet Analysis</button>
      <button data-action="author">Account Info</button>
      <button data-action="details">${sourceLabel === "TRACKED POST" ? "Show tracked data" : "Show more"}</button>
    </div>
  </article>`;
}

async function handlePostAction(id, event, source = "scan") {
  const action = event.target.dataset.action;
  if (!action) return;
  const post = findPostBySource(id, source);
  if (!post) return;
  if (action === "details") showRecordDialog("Tweet Details", post, source === "tracker" ? "TRACKED POST" : "SEARCH RESULT");
  if (action === "author") {
...
function findPostBySource(id, source) {
  const collection = source === "tracker" ? state.trackerPosts : state.posts;
  return collection.find(item => item.id === id) || state.posts.find(item => item.id === id) || state.trackerPosts.find(item => item.id === id);
}
...
async function loadTweetDetail() {
  ...
  $("#tweet-detail-excel").disabled = true;
  $("#tweet-detail-pdf").disabled = true;
  ...
  try {
    ...
    $("#tweet-detail-excel").disabled = false;
    $("#tweet-detail-pdf").disabled = false;
...
async function exportTweetDetailExcel() {
  if (!state.tweetDetail?.tweet?.id) return showError("Load tweet details before exporting.");
  const id = state.tweetDetail.tweet.id;
  const path = await window.trexDesktop.saveFile({
    defaultPath: `tweet_detail_${id}.xlsx`,
    filters: [{ name: "Excel Files", extensions: ["xlsx"] }]
  });
  if (!path) return;
  showProgress("Export Tweet Detail", "Writing structured TweetDetail workbook…", 25);
  try {
    await api("/api/export/tweet-detail", { method: "POST", body: { path, record: state.tweetDetail } });
    updateProgressDialog("TweetDetail Excel export complete.", 100);
    await wait(500);
    closeProgress();
  } catch (error) {
...
async function exportTweetDetailPDF() {
  if (!state.tweetDetail?.tweet?.id) return showError("Load tweet details before exporting.");
  const id = state.tweetDetail.tweet.id;
  showProgress("Export Tweet Detail PDF", "Preparing decorated TweetDetail report…", 25);
  const html = tweetDetailReportHTML(state.tweetDetail);
  updateProgressDialog("Rendering TweetDetail PDF report…", 70);
  const path = await window.trexDesktop.savePDF({ html, defaultPath: `tweet_detail_${id}.pdf` });
  if (path) {
    updateProgressDialog("TweetDetail PDF export complete.", 100);
    await wait(500);
  }
  closeProgress();
}
...
function handleUpdateEvent(event) {
  ...
  if (event.type === "ready-to-install") {
    showUpdateDialog({
      title: "Update downloaded",
      message: `T-REX OSINT ${event.newVersion} is ready to install. The app must close before installing so Windows can replace the files safely.`,
      status: "Ready to install after closing T-REX OSINT.",
      progress: 100,
      downloadEnabled: true,
      downloadText: "Install & Close App",
      laterText: "Later"
    });
  }
  if (event.type === "installing-after-close") {
    showUpdateDialog({
      title: "Installing after close",
      message: "T-REX OSINT will close now. The installer will continue silently in the background. Reopen the app after a few moments.",
      status: "Closing application and starting installer...",
      progress: 100,
      downloadEnabled: false,
      laterText: "Close"
    });
...
function showUpdateDialog({ title, message, status, progress, downloadEnabled, downloadText, laterText }) {
  ...
  $("#update-download").textContent = downloadText || "Download Update";
...
async function downloadAndInstallUpdate() {
  ...
  if ($("#update-download").textContent.includes("Install")) {
    try {
      await window.trexDesktop.installDownloadedUpdate();
    } catch (error) {
...
function tweetDetailReportHTML(record) {
  const tweet = record.tweet || {};
  const author = tweet.author || {};
  const metrics = tweet.metrics || {};
  const username = author.screen_name || "unknown";
  const avatar = author.profile_image_url || author.profile_image_url_https;
  const metricCards = [
  ...
  return reportDocument(`TweetDetail Report ${tweet.id || ""}`, `
    <section class="hero account-hero">
      ${avatar ? `<img src="${escapeAttribute(avatar)}">` : ""}
      <div>
        <p>Generated ${new Date().toLocaleString()}</p>
        <h1>TweetDetail Intelligence Report</h1>
        <h3>@${escapeHTML(username)} · ${escapeHTML(tweet.createdAt || "")}</h3>
      </div>
    </section>
    <section><h2>Focal Tweet</h2><div class="tweet-box">
      <p>${escapeHTML(tweet.text || "")}</p>
      ${tweet.url ? `<p><a href="${escapeAttribute(tweet.url)}">${escapeHTML(tweet.url)}</a></p>` : ""}
    </div></section>
    <section><h2>Engagement Snapshot</h2><div class="stats">${metricCards.map(([label, value]) => stat(label, value)).join("")}</div></section>
    ${(record.sections || []).map(section => `<section><h2>${escapeHTML(section.title)}</h2>${table(["Field", "Value"], (section.rows || []).map(row => [row.label, printable(row.value)]))}</section>`).join("")}
    ${tweet.raw ? `<section><h2>Raw Tweet Fields</h2>${table(["Field", "Value"], Object.entries(flatten(tweet.raw)).map(([key, value]) => [humanize(key), printable(value)]))}</section>` : ""}
  `);
}
...
function reportDocument(title, content) {
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHTML(title)}</title><style>
    ...
    .tweet-box{padding:14px 16px;border:1px solid #d5dadd;border-radius:8px;background:#f5f6f7;white-space:pre-wrap}.tweet-box p{margin:0 0 8px}
