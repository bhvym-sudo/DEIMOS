...
def load_session_cookies(runtime_path):
    with open(runtime_path, "r", encoding="utf-8") as handle:
        runtime = json.load(handle)
    cookies = []
    for source in runtime.get("cookies", []):
        if not source.get("name") or not source.get("value") or not source.get("domain"):
            continue
        cookie = {
            "name": source["name"], "value": source["value"],
            "domain": source["domain"], "path": source.get("path") or "/",
            "httpOnly": bool(source.get("httpOnly")), "secure": bool(source.get("secure")),
        }
        if float(source.get("expires") or 0) > 0:
            cookie["expires"] = float(source["expires"])
        same_site = str(source.get("sameSite") or "").lower()
...
async def collect(request, profile_dir=None, runtime_path=None):
    ...
    if not runtime_path and not os.path.isdir(profile_dir or ""):
        raise RuntimeError(f"X Edge profile does not exist: {profile_dir}")
    if runtime_path and not os.path.isfile(runtime_path):
        raise RuntimeError(f"X session file does not exist: {runtime_path}")
    ...
    async with async_playwright() as playwright:
        browser = None
        if runtime_path:
            emit("status", message="Opening authenticated Docker Chromium session...", progress=5)
            ...
            context = await playwright.chromium.launch_persistent_context(
                ...
                args=["--headless=new", "--disable-gpu", "--disable-extensions", "--disable-notifications", "--disable-component-update", "--no-first-run", "--no-default-browser-check"],
            )
        ...
        if not any(cookie.get("name") == "auth_token" for cookie in cookies):
            await context.close()
            raise RuntimeError("the supplied X session does not contain an authenticated auth_token cookie")
...
async def main():
    ...
    await collect(
        request,
        os.path.abspath(args.profile) if args.profile else None,
        os.path.abspath(args.runtime) if args.runtime else None,
    )
